...
parser.add_argument("--target", help="specify the save location directory of the xml files", default='Corrected-annotations', type=str)
args = parser.parse_args()

file_list = glob.glob(args.source + "/*.xml")

if not (os.path.isdir('Corrected-annotations')):
    os.mkdir("Corrected-annotations")
...
for file in file_list:
    ...
    with open(file,'r') as f:
        ...
        for line in text:
            # change folder
            if line.find("<folder>Annotations-labelimg")>0:
                i+=1
                line="\t<folder>plants</folder>\n"
            elif line.find("<database>Unknown")>0:
                line="\t\t<database>plants</database>\n\t\t<annotation>custom</annotation>\n\t\t<image>custom</image>\n"
